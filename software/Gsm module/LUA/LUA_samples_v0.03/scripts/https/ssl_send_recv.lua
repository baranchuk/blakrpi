function cch_check_recv_data(session_id, timeout)
  local start_tick = os.clock();
  while (true) do
    local cached_rx_len = common_ch.get_rx_cache_len(session_id);
...
function cch_read_all_cached_rcvd_data(session_id, timeout)
  while (true) do
	  if (cch_check_recv_data(session_id, timeout)) then
...
--set network retransmission parameter, by using following setting, the TCP may run better 
network.set_tcp_retran_param(10, 10000);
...
if (error_code and (error_code == CCH_RST_OK)) then  
  ...
  for i = 1, 10000, 1 do    
    local send_data = "\r\nSEQ( "..i.." ) "..data;
	print("test send ", i, " times\r\n");
    error_code = common_ch.send(session_id, send_data);
    print("common_ch.send(), error_code=", error_code, "\r\n");
	if (error_code and (error_code == CCH_RST_OK)) then  
	else
	  print("send failed, break now\r\n");
	  break;
	end;
	cch_read_all_cached_rcvd_data(session_id, 0);	
  end;
  --check whether it has any more rx packet, waiting 15 seconds
  if (error_code and (error_code == CCH_RST_OK)) then    
	print("wait for recv event\r\n");
	local timeout = 15000;
	print("now wait for RECV event,timeout=", timeout, "\r\n");
	cch_read_all_cached_rcvd_data(session_id, timeout);
  end;
